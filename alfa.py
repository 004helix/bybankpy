...
class alfa:
    ...
    def gate(self, service, operation, params={}):
        ...
        if result.service == 'Exception':
            exc = u'%s: %s' % (result.header['faultCode'],
                               result.header['faultMessage'])
            raise AlfaException(exc.encode('utf-8'))
    ...
    def transfer(self, fromid, toid, amount, currency):
        ...
        rq = "<?xml version='1.0' ?>"
        rq += '<d o="336"><f1>%s</f1><f2>%s</f2><f3>%s</f3><f4>%s</f4></d>' % \
              (str(fromid), str(toid), str(amount), str(currency))

        repl = lxml.objectify.fromstring(self._ctrl(0x0d, rq))
        ...
        rq = "<?xml version='1.0' ?>"
        rq += '<d o="%s"><f1>%s</f1><f2>%s<f2><f4>%s</f4></d>' % \
              (opid, flds['f1'], flds['f2'], flds['f4'])

        return self._ctrl(0x0e, rq)
    ...
    def exchange(self, fromid, toid, amount, currency):
        ...
        rq = "<?xml version='1.0' ?>"
        rq += '<d o="313"><f1>%s</f1><f2>%s</f2><f3>%s</f3><f4>%s</f4></d>' % \
              (str(fromid), str(toid), str(amount), str(currency))
    ...
    # cached version of summary
    # (accounts, credits and deposits, but no difference between them)
    def get_common_accounts(self):
        return self.gate('Budget', 'GetCommonAccounts',
                         {"operation": "mainPage"}).accounts
    ...
    def account_statement(self, number):
        ...
        d = lxml.objectify.fromstring(self._ctrl(0x04, rq))
        ...
        for r in statement:
            if int(r['date'].replace('-', '')) < m:
                break
            rq = "<?xml version='1.0' ?>"
            rq += '<d><f>%s</f></d>' % str(r['id'])
            d = lxml.objectify.fromstring(self._ctrl(0x0a, rq))
